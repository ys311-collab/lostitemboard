...
def start():
    ...
    def lost_input(ml): #등록하기 버튼 (Submit) 을 누르면 실행되는 함수
        ...
        ttk.Button(button_bar,text='Take this!!',width=10, height=4, bd=0,padx=0,pady=0, command=lambda: load('laf',frm)).pack(side=tk.LEFT)
        ttk.Button(button_bar,text='Find this!!',width=10, height=4, bd=0,padx=0,pady=0, command=lambda: load('cag',frm)).pack(side=tk.LEFT)
    ...
    def to_home():
        ...
        typ='lf'
        reload_data(typ)
    ttk.Button(util_frm,text='HOME',command=to_home).pack(side='left',anchor='center',padx=10)
    ...
    def login(ml):
        ...
        ttk.Button(window, text="Login", command=check_login).pack(pady=5)
    ...
    def create_id(ml):
        ...
        def register():
            ...
            if username in users:
                ...
                window.destroy()
                update_login_menu()
        ttk.Button(window, text="Create ID", command= register).pack(pady=5)
    ...
    thememenu.add_command(label="Modern Basic", command = basic_theme) #모두 theme의 함수
